...
import { Heart, History, Calendar, ChevronLeft, ChevronRight } from 'lucide-react';
...
// Định nghĩa cấu trúc Item sản phẩm (bạn có thể chỉnh sửa lại các field cho đúng thực tế nếu cần)
interface ProductItem {
  id: string;
  [key: string]: unknown;
}

// Định nghĩa cấu trúc dữ liệu Phân trang chuẩn từ Backend trả về
...
export default function InteractionsPage() {
  ...
  const { data: favoritesData, isLoading: loadingFavorites } = useQuery({
    ...
    queryFn: async () => {
      const res = await userService.getFavorites(page, 15, dates.startDate, dates.endDate);
  ...
  const { data: viewedData, isLoading: loadingViewed } = useQuery({
    ...
    queryFn: async () => {
      const res = await userService.getRecentlyViewed(page, 15, dates.startDate, dates.endDate);
      return res as PaginatedResponse;
    },
  });

  const renderPagination = (data: PaginatedResponse | undefined) => {
      ...
      >
        ...
        <TabsContent value="favorites" className="mt-0">
          {loadingFavorites ? (
            ...
            <>
              <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-5">
                {favoritesData.content.map((product) => (
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center bg-white/40 rounded-3xl border border-dashed backdrop-blur-sm">
              ...
              <p className="text-muted-foreground font-medium">
                Không tìm thấy sản phẩm yêu thích nào trong khoảng thời gian này.
              </p>
            </div>
          )}
        </TabsContent>
        ...
        <TabsContent value="viewed" className="mt-0">
          {loadingViewed ? (
            ...
            <>
              <div className="grid grid-cols-2 gap-4 md:grid-cols-4 lg:grid-cols-5">
                {viewedData.content.map((product) => (
                  <ProductCard key={product.id} product={product as unknown as Product} />
                ))}
              </div>
            ...
            <div className="flex flex-col items-center justify-center py-20 text-center bg-white/40 rounded-3xl border border-dashed backdrop-blur-sm">
              ...
              <p className="text-muted-foreground font-medium">
                Không tìm thấy sản phẩm đã xem nào trong khoảng thời gian này.
              </p>
            </div>
          )}
        </TabsContent>
