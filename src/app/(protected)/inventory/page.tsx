...
import {
  ...
  useNearExpiryAlerts,
  useExpiredAlerts,
  useExportExpiredToDoctor,
  useExportAllExpiredToDoctor,
} from '@/features/inventory/hooks/use-inventory';
import Link from 'next/link';

export default function InventoryDashboardPage() {
  const { data: dashboard, isLoading: dashLoading } = useInventoryDashboard();
  const { data: nearExpiryBatches = [], isLoading: nearLoading } = useNearExpiryAlerts('STORAGE');
  const { data: expiredBatches = [], isLoading: expiredLoading } = useExpiredAlerts('STORAGE');
  const exportExpiredMutation = useExportExpiredToDoctor();
  const exportAllExpiredMutation = useExportAllExpiredToDoctor();
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 md:p-[48px] bg-m3-background h-full">
      <div className="mb-[40px] flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-2">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-800 dark:bg-emerald-950/60 dark:text-emerald-300 border border-emerald-300/40">
              <Warehouse className="w-3.5 h-3.5" />
              Kho Bảo Quản (Thủ kho)
            </span>
          </div>
        ...
        {expiredBatches.length > 0 && (
          <button
            type="button"
            disabled={exportAllExpiredMutation.isPending}
            onClick={() => exportAllExpiredMutation.mutate()}
            className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-red-600 text-white font-medium text-sm hover:bg-red-700 active:scale-[0.98] transition-all shadow-md self-start md:self-auto disabled:opacity-60 cursor-pointer"
          >
            <Send className="w-4 h-4" />
            <span>
              {exportAllExpiredMutation.isPending
                ? 'Đang xuất kho...'
                : `Xuất tất cả ${expiredBatches.length} lô hết hạn lên kho bác sĩ`}
            </span>
          </button>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-[24px]">
        ...
        <div className="md:col-span-8 grid grid-cols-1 sm:grid-cols-2 gap-[24px]">
          ...
          <div className="bg-m3-surface rounded-xl p-[24px] shadow-[0_4px_20px_rgba(0,0,0,0.04)] border border-m3-surface-container-high flex flex-col justify-between">
            <div className="flex justify-between items-start mb-4">
              <div>
                <p className="font-label-md text-[14px] font-semibold text-m3-on-surface-variant mb-1">
            ...
            {totalItems > 0 ? (
              ...
              <Link
                ...
                className="inline-flex items-center gap-2 text-m3-primary font-label-sm text-[13px] font-semibold hover:underline w-fit bg-m3-primary/10 px-3 py-1.5 rounded-full mt-2"
              >
          ...
          {/* Low Stock Alert */}
          <div
            className={`bg-m3-surface rounded-xl p-[24px] shadow-[0_4px_20px_rgba(0,0,0,0.04)] border flex flex-col justify-between relative overflow-hidden ${
          ...
          >
            ...
            <div className="flex justify-between items-start mb-4 relative z-10">
              <div>
                <p
                  className={`font-label-md text-[14px] font-semibold mb-1 ${
                ...
                <h3
                  className={`font-display-lg text-[48px] font-bold ${
                    totalAlerts > 0 ? 'text-m3-error' : 'text-m3-primary'
                  }`}
                >
            ...
            <div className="flex items-center gap-2 text-m3-on-surface-variant font-label-sm text-[12px] font-medium relative z-10">
              <span>
                {totalAlerts > 0
                  ? 'Cần kiểm tra lại các lô cận date và hết hàng'
                  : 'Tuyệt vời! Kho đang ở trạng thái an toàn'}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="md:col-span-4 bg-m3-surface rounded-xl p-[24px] shadow-[0_4px_20px_rgba(0,0,0,0.04)] border border-m3-surface-container-high flex flex-col items-center justify-center">
          <h3 className="font-label-md text-[14px] font-semibold text-m3-on-surface w-full mb-6 text-left">
            Phân Bổ Kho
          </h3>
          ...
          <div className="relative w-40 h-40 mb-6">
            <svg className="w-full h-full transform -rotate-90" viewBox="0 0 100 100">
              <circle
                ...
                fill="none"
                r="45"
                stroke="var(--color-m3-surface-variant)"
                strokeWidth="10"
              ></circle>
              {totalItems > 0 && (
                <>
            ...
            <div className="absolute inset-0 flex flex-col items-center justify-center">
              <span className="font-headline-md text-[24px] font-semibold text-m3-on-surface">
                {totalItems > 0 ? '100%' : '0'}
              </span>
              <span className="font-label-sm text-[12px] text-m3-on-surface-variant">
                {totalItems > 0 ? 'Tổng Dung Lượng' : 'Chưa có dữ liệu'}
              </span>
            </div>
          </div>
          {totalItems > 0 && (
        ...
        <div className="md:col-span-12 bg-m3-surface rounded-xl p-[24px] shadow-[0_4px_20px_rgba(0,0,0,0.04)] border border-m3-surface-container-high">
          <div className="flex justify-between items-center mb-4">
            <h3 className="font-label-md text-[14px] font-semibold text-m3-on-surface">
              Lối Tắt Nhanh
            </h3>
            <Link
              href="/inventory/vouchers"
              className="text-m3-primary font-label-md text-[14px] font-semibold hover:underline"
            >
              Lịch Sử Chi Tiết
            </Link>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Link
              href="/inventory/vouchers/new"
              className="flex items-center gap-3 p-4 rounded-xl hover:bg-m3-surface-container-low transition-colors border border-m3-outline-variant/50"
            >
              <div className="p-3 rounded-full bg-m3-primary/10 text-m3-primary">
                <ArrowDownRight className="w-5 h-5" />
              </div>
              <div>
                <p className="font-label-md text-[14px] font-semibold text-m3-on-surface">
                  Nhập Kho Mới
                </p>
        ...
        <div className="md:col-span-12 bg-m3-surface rounded-xl shadow-[0_4px_20px_rgba(0,0,0,0.04)] border border-m3-surface-container-high overflow-hidden">
          <div className="p-4 border-b border-m3-surface-container-high flex justify-between items-center bg-m3-surface-bright flex-wrap gap-2">
            <h3 className="font-label-md text-[14px] font-semibold text-m3-on-surface">
              Danh Sách Cần Chú Ý (Lô Cận Date/Hết Hạn)
            </h3>
          ...
          <div className="flex flex-col">
            {nearLoading || expiredLoading ? (
              ...
              <div className="flex flex-col items-center justify-center p-8 text-center">
                ...
                <p className="font-label-lg text-m3-on-surface mb-2 font-medium">
                  Kho hàng đang an toàn
                </p>
                <p className="font-label-sm text-m3-on-surface-variant mb-6 max-w-sm">
                  Hệ thống sẽ tự động nhắc nhở khi có vật tư sắp hết hạn hoặc tồn kho thấp. Hãy bắt
                  đầu quản lý kho ngay!
                </p>
                <Link
                  href="/inventory/vouchers/new"
                  className="bg-m3-primary text-m3-on-primary px-5 py-2 rounded-full font-label-md font-semibold hover:bg-m3-primary/90 transition-colors"
                >
                  Tạo phiếu nhập kho
                </Link>
              </div>
              ...
              [...(expiredBatches || []), ...(nearExpiryBatches || [])]
                ...
                .map((batch, idx) => (
                  <div
                    key={idx}
                    className="flex items-center justify-between p-4 border-b border-m3-surface-container-high hover:bg-m3-surface-container-low transition-colors flex-wrap gap-3"
                  >
                    ...
                    <div className="flex items-center gap-4 sm:gap-6">
                      ...
                      {batch.isExpired && (
                        <button
                          type="button"
                          disabled={exportExpiredMutation.isPending}
                          onClick={() => exportExpiredMutation.mutate(batch.id)}
                          className="px-2.5 py-1 text-xs font-semibold rounded-lg bg-red-600 text-white hover:bg-red-700 disabled:opacity-50 transition-colors shadow-sm whitespace-nowrap flex items-center gap-1 cursor-pointer"
                        >
                          <Send className="w-3 h-3" />
                          <span>Xuất lên kho BS</span>
                        </button>
                      )}
                    </div>
                  </div>
