...
export function VoucherForm() {
  ...
  const [sourceWarehouse, setSourceWarehouse] =
    useState<import('@/types/inventory').WarehouseLocation>('STORAGE');
  const [destinationWarehouse, setDestinationWarehouse] = useState<
    import('@/types/inventory').WarehouseLocation | ''
  >('');
  ...
  const updateItem = (idx: number, updates: Partial<VoucherItemRequest>) => {
    setItems((prev) => {
      const updated = [...prev];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const req: CreateVoucherRequest = {
      ...
      destinationWarehouse:
        (destinationWarehouse as import('@/types/inventory').WarehouseLocation) || undefined,
      note: note || undefined,
  ...
  const calculateTotal = () => {
    return items.reduce(
      (acc: number, item: VoucherItemRequest) => acc + item.quantity * (item.unitPrice || 0),
      0
    );
  };
        ...
        <div className="max-w-5xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-[24px]">
          <div>
            <h2 className="font-headline-lg-mobile md:font-headline-lg text-[24px] md:text-[32px] font-semibold text-m3-on-surface mb-1">
              {type === 'IMPORT'
                ? 'Nhập Kho Mới (Kho bảo quản)'
                : type === 'EXPORT'
                  ? 'Xuất Kho (Kho bảo quản → Kho bác sĩ / Tiêu hao)'
                  : type === 'TRANSFER'
                    ? 'Chuyển Kho Nội Bộ'
                    : 'Phiếu Kiểm Kê'}
            </h2>
            <p className="font-body-md text-[16px] text-m3-on-surface-variant">
              Ghi nhận giao dịch tồn kho giữa Kho bảo quản và Kho bác sĩ.
            </p>
          </div>
      ...
      <div className="flex-1 px-4 md:px-[48px] py-[24px] md:py-[40px] overflow-y-auto">
        <div className="max-w-5xl mx-auto space-y-[24px]">
          ...
          <section className="bg-m3-surface-container-lowest p-[24px] rounded-xl shadow-[0px_4px_20px_rgba(0,0,0,0.04)] border border-m3-outline-variant/20">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-[12px]">
              <div className="space-y-[4px]">
                <label className="font-label-sm text-[12px] text-m3-on-surface-variant">
                  Loại phiếu <span className="text-m3-error">*</span>
                </label>
                <select
                  value={type}
                  onChange={(e) => {
              ...
              {(type === 'EXPORT' || type === 'TRANSFER') && (
                <div className="space-y-[4px]">
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setDestinationWarehouse(
                        e.target.value as import('@/types/inventory').WarehouseLocation | ''
                      )
                    }
                    className="w-full px-[12px] py-[12px] bg-m3-surface border border-m3-primary/40 rounded-lg font-body-md text-[16px] text-m3-primary focus:border-m3-primary focus:ring-2 focus:ring-m3-primary/10 transition-all outline-none font-medium"
                  >
              ...
              <div className="space-y-[4px]">
                ...
                <input
                  ...
                  placeholder="VD: Xuất lô thuốc sang kho bác sĩ..."
                  className="w-full px-[12px] py-[12px] bg-m3-surface border border-m3-outline-variant rounded-lg font-body-md text-[16px] text-m3-on-surface focus:border-m3-primary focus:ring-2 focus:ring-m3-primary/10 transition-all outline-none"
                />
              </div>
            </div>
          ...
          <div className="flex items-center justify-between mb-2">
            <h3 className="font-headline-md text-[24px] font-semibold text-m3-on-surface">
              Chi tiết mặt hàng
            </h3>
            <button
              type="button"
          ...
          {items.map((item: VoucherItemRequest, idx: number) => (
            <div
              key={idx}
              className="grid grid-cols-1 md:grid-cols-12 gap-[24px] pb-6 border-b border-m3-outline-variant/30 last:border-0 relative"
            >
              {/* Main Form Column */}
              <div className="md:col-span-8 space-y-[24px]">
                ...
                <section className="bg-m3-surface-container-lowest p-[24px] rounded-xl shadow-[0px_4px_20px_rgba(0,0,0,0.04)] border border-m3-outline-variant/20">
                  <h4 className="font-headline-md text-[18px] font-semibold text-m3-on-surface mb-[12px] flex items-center gap-[12px]">
                    <span className="text-m3-primary bg-m3-primary-container/20 p-[4px] rounded-full">
                  ...
                  <div className="space-y-[12px]">
                    <div className="space-y-[4px]">
                      <label className="font-label-sm text-[12px] text-m3-on-surface-variant">
                        Sản phẩm / Vật tư <span className="text-m3-error">*</span>
                      </label>
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          updateItem(idx, {
                            medicineId: val,
                            productId: val ? undefined : undefined,
                          });
                        }}
                        className="w-full px-[12px] py-[12px] bg-m3-surface border border-m3-outline-variant rounded-lg font-body-md text-[16px] text-m3-on-surface focus:border-m3-primary focus:ring-2 focus:ring-m3-primary/10 transition-all outline-none"
                      >
                ...
                {type === 'IMPORT' && (
                  <section className="bg-m3-surface-container-lowest p-[24px] rounded-xl shadow-[0px_4px_20px_rgba(0,0,0,0.04)] border border-m3-outline-variant/20">
                    <h4 className="font-headline-md text-[18px] font-semibold text-m3-on-surface mb-[12px] flex items-center gap-[12px]">
                      <span className="text-m3-tertiary-container bg-m3-tertiary-container/20 p-[4px] rounded-full">
              ...
              <div className="md:col-span-4 space-y-[24px]">
                <section className="bg-m3-surface-container-lowest p-[24px] rounded-xl shadow-[0px_4px_20px_rgba(0,0,0,0.04)] border border-m3-outline-variant/20 h-full flex flex-col relative">
                  ...
                  <h4 className="font-headline-md text-[18px] font-semibold text-m3-on-surface mb-[12px] flex items-center gap-[12px]">
                    <span className="text-m3-secondary bg-m3-secondary/20 p-[4px] rounded-full">
                  ...
                  <div className="space-y-[12px] flex-1 mt-4">
                    <div className="space-y-[4px]">
                      <label className="font-label-sm text-[12px] text-m3-on-surface-variant">
                        Số lượng <span className="text-m3-error">*</span>
                      </label>
                      ...
                      <div className="space-y-[4px] pt-[4px]">
                        <label className="font-label-sm text-[12px] text-m3-on-surface-variant">
                          Đơn giá (VNĐ) <span className="text-m3-error">*</span>
                        </label>
                        <div className="relative">
                          <input
                            type="number"
                          ...
                          />
                          <span className="absolute right-[12px] top-1/2 -translate-y-1/2 text-m3-on-surface-variant font-label-md text-[14px]">
                            ₫
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="mt-auto pt-[24px] border-t border-m3-outline-variant/50">
                        <div className="flex justify-between items-center mb-[4px]">
                          <span className="font-body-sm text-[14px] text-m3-on-surface-variant">
                            Tạm tính:
                          </span>
          ...
          <div className="bg-m3-surface-container-lowest p-[24px] rounded-xl shadow-[0px_4px_20px_rgba(0,0,0,0.04)] border border-m3-outline-variant/20 flex justify-between items-center">
            ...
            <span className="font-display-lg text-[32px] font-bold text-m3-primary">
