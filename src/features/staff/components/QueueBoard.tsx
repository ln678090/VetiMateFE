...
export const QueueBoard = ({ type }: QueueBoardProps) => {
  const { tickets, isLoading, createTicket, updateStatus, isCreating, isUpdatingStatus } =
    useQueue(type);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-3 min-h-[500px] bg-slate-50/50 p-3 rounded-b-xl border border-slate-100 border-t-0">
            ...
            {waiting.length === 0 && (
              <p className="text-sm text-slate-400 text-center py-4">Trống</p>
            )}
          </div>
        </div>
  ...
  return (
    <Card className="p-4 shadow-sm border-slate-200">
      ...
      {ticket.customerName ? (
        <div className="mt-3 text-sm space-y-1">
          ...
          {ticket.petName && (
            <div className="text-slate-500">
              Thú cưng: <span className="font-medium text-slate-700">{ticket.petName}</span>
            </div>
          )}
          {ticket.serviceName && (
        ...
        <div className="mt-3 text-sm italic text-slate-500">Khách vãng lai (Chờ khám)</div>
      )}
